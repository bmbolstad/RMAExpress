 ...
 ** Apr 22, 2003 - fix computation of bandwidth, add in linear interpolation step 
 ** Mar 24, 2005 - Add in IQR function to handle obscure cases.
 ** Oct 5, 2006 - Added in a sum specific include as suggested by the
 **               LCB BASE team.
 ** Jan 21, 2007 - add KernelDensity_lowmem and unweighted_massdist code
 ** Feb 6, 2008 - change qsort to stl::sort
 **
 ****************************************************************************/
...
void KernelDensity(double *x, int *nxxx, double *weights, double *output, double *output_x, int *nout){
  ...
  double *buffer = (double *)calloc(nx,sizeof(double));
